...
public class CombineSkinnedUtil {
...
//			{
//				if(!materials.Contains(smr.sharedMaterials[j]))
//					materials.Add(smr.sharedMaterials[j]);
//			}
		...
		if (oldSKinned != null) {
            GameObject.DestroyImmediate (oldSKinned,false);
		}

        
		SkinnedMeshRenderer r = skeleton.AddComponent<SkinnedMeshRenderer>();
        Mesh m = new Mesh();
        m.CombineMeshes(combineInstances.ToArray(), combine, false);

        r.sharedMesh = m;
		//r.sharedMesh// Combine meshes
